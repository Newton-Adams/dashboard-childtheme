<form id="contact-fields" >
    <div class="form-row" >
        <div class="d-flex flex-column fw-100" >
            <p>Contact 1</p>
            <div class="fw-100 pr-2" >
                <label for="first-name-1" >First Name</label>
                <input type="text" id="first-name-1" name="first-name-1" >
            </div>
            <div class="fw-100 pr-2" >
                <label for="last-name-1" >Last Name</label>
                <input type="text" id="last-name-1" name="last-name-1" >
            </div>
            <div class="fw-100 pr-2" >
                <label for="email-1" >Email</label>
                <input type="text" id="email-1" name="email-1" >
            </div>
            <div class="fw-100 pr-2" >
                <label for="cell-number-1" >Cell number</label>
                <input type="text" id="cell-number-1" name="cell-number-1" >
            </div>
            <div class="fw-100 pr-2" >
                <label for="additional-number-1" >Additional cell number (Whatsapp)</label>
                <input type="text" id="additional-number-1" name="additional-number-1" >
            </div>
        </div>
        <div class="d-flex flex-column fw-100" >
            <p>Contact 2</p>
            <div class="fw-100" >
                <label for="first-name-2" >First Name</label>
                <input type="text" id="first-name-2" name="first-name-2" >
            </div>
            <div class="fw-100" >
                <label for="last-name-2" >Last Name</label>
                <input type="text" id="last-name-2" name="last-name-2" >
            </div>
            <div class="fw-100" >
                <label for="email-2" >Email</label>
                <input type="text" id="email-2" name="email-2" >
            </div>
            <div class="fw-100" >
                <label for="cell-number-2" >Cell number</label>
                <input type="text" id="cell-number-2" name="cell-number-2" >
            </div>
            <div class="fw-100" >
                <label for="additional-number-2" >Additional cell number (Whatsapp)</label>
                <input type="text" id="additional-number-2" name="additional-number-2" >
            </div>
        </div>
    </div>
</form>