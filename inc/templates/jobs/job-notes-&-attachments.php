<?php 
 !empty(get_post_meta(386, 'attachments', true) ) && $attachments = get_post_meta(386, 'attachments', true); 
 !empty($attachments) && $attchments_decoded = json_decode($attachments);
 echo '<pre>',print_r(json_decode($attachments)),'</pre>';
    ...
    <div class="form-row attachments" >
        <div class="d-flex flex-column fw-100" >
            ...
            <div class="fw-100" >
                ...
                <input type="hidden" id="attachments-obj" class="d-none" value='<?= json_encode($existingAttachments); ?>' >                
            </div>
        </div>
    </div>
</form>
<table id="attachment-files" >
    ...
    <?php 
    foreach ($attchments_decoded as $key => $attachment) {
        echo '<td><a href="'.$attachment[1].'" >'.$attachment[0].'</a></td>';
        echo '<td class="delete" attachment_id="'.$attachment[2].'" >
