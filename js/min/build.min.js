...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '#save-post.job-save', function (event) {
    ...
    //Cost
    var jobGrandTotal = $("input#grand-total-value").val();
  ...
  $(document).on('click', '.edit-vehicle-action', function (e) {
    ...
    $.ajax({
      ...
      success: function success(response) {
        ...
        var customerData = '{"customer-name":"' + vehicleData['customer-name'] + '","company-name":"' + vehicleData['company-name'] + '","email":"' + vehicleData['email'] + '","phone":' + vehicleData['phone'] + ',"address":"' + vehicleData['address'] + '"}';
  ...
  $(document).on('click', '.delete-vehicle-action', function (e) {
    ...
    var tr_post_id = $(this).closest('tr').attr('class');
    // Strip 'post-id-' from tr_post_id 
    var delete_post_id = tr_post_id.replace('post-id-', '');
    $.ajax({
      url: workshop_pro_obj.ajaxurl,
      type: 'POST',
      data: {
...
jQuery(document).ready(function ($) {
  ...
  $(document).on('click', '.select-wrapper.mechanics .options > span', function () {
    var self = $(this);
    var selectedMechanics = $('#selected-mechanics').val().length > 0 ? JSON.parse($('#selected-mechanics').val()) : '';
  ...
  $(document).on('click', '.select-wrapper-outer .assigned-mechanics-outer .assigned-mechanics .mechanic-inner .close', function () {
    var self = $(this);
    var selectedMechanics = JSON.parse($('#selected-mechanics').val());
    var removedMechanic = self.closest('.mechanic').data('mechanic').trim();
  ...
  if ($("#main > .editing").length > 0) {
    ...
    $('.section.labour-container .sub-total-outer .sub-total .value').text("R".concat(labourLineTotal.toFixed(2)));
    ...
    $('.section.parts-container .sub-total-outer .sub-total .value').text("R".concat(partLineTotal.toFixed(2)));
